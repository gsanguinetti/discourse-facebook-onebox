...
module Onebox
  module Engine
    class FacebookOnebox
      ...
      def get_oembed_data
        if video?
          Onebox::Helpers.symbolize_keys(::MultiJson.load(Onebox::Helpers.fetch_response("http://localhost:10000/proxy/headervn/https://facebook.com/plugins/video/oembed.json?url=#{url}&maxwidth=661&omitscript=true").body))
        else
          Onebox::Helpers.symbolize_keys(::MultiJson.load(Onebox::Helpers.fetch_response("http://localhost:10000/proxy/headervn/https://facebook.com/plugins/post/oembed.json?url=#{url}&maxwidth=661&omitscript=true").body))
        end
      end
    end
  end
